        ...
        if enable_long:
            ...
            if(strategy_long(data_feed) == -1 and num_open_trades(long_trades) == 1):
                ...
                long_trades.loc[exit_index, ['Exit Date', 'Exit Price', 'Return', 'Total Return', 'Run-up', 'Drawdown']] = [
                    ...
                    calc_returns(long_trades.at[exit_index, 'Entry Price'], data_feed['Close'].iloc[-1], True),
                    long_trades.at[exit_index-1, 'Total Return'] * (1 + calc_returns(long_trades.at[exit_index, 'Entry Price'], data_feed['Close'].iloc[-1], True)),
        ...
        if enable_short:
            ...
            if(strategy_short(data_feed) == -1 and num_open_trades(short_trades) == 1):
                ...
                short_trades.loc[exit_index, ['Exit Date', 'Exit Price', 'Return', 'Total Return', 'Run-up', 'Drawdown']] = [
                    data_feed.index[-1],
                    data_feed['Close'].iloc[-1],
                    calc_returns(short_trades.at[exit_index, 'Entry Price'], data_feed['Close'].iloc[-1], False),
                    short_trades.at[exit_index-1, 'Total Return'] + calc_returns(short_trades.at[exit_index, 'Entry Price'], data_feed['Close'].iloc[-1], False),
